...
export async function resolveAndFetchPendingVisits(userId: string): Promise<ResolvedVisit[]> {
  ...
  const otherIds = Array.from(new Set(
    (visits as Record<string, any>[]).map((v) => v.host_user_id === userId ? v.visitor_user_id : v.host_user_id)
  ));
  ...
  const result: ResolvedVisit[] = (visits as Record<string, any>[]).map((v) => {
  ...
  const hostIds = (visits as Record<string, any>[]).filter((v) => v.host_user_id === userId).map((v) => v.id);
  const visitorIds = (visits as Record<string, any>[]).filter((v) => v.visitor_user_id === userId).map((v) => v.id);
  if (hostIds.length > 0) {
...
export async function fetchAndMarkUnseenNotifications(userId: string): Promise<PetNotification[]> {
  ...
  const fromIds = Array.from(new Set((data as Record<string, any>[]).map((n) => n.from_user_id).filter(Boolean)));
  ...
  const ids = (data as Record<string, any>[]).map((n) => n.id);
