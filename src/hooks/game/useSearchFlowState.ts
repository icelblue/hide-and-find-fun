...
export function useSearchFlowState() {
  ...
  const [connectedScenarios, setConnectedScenarios] = useState<Record<string, any>[]>([]);
  const [moveHistory, setMoveHistory] = useState<Record<string, any>[]>([]);
  const [actionLoading, setActionLoading] = useState(false);
  const [itemInteractions, setItemInteractions] = useState<Record<string, any>[]>([]);
