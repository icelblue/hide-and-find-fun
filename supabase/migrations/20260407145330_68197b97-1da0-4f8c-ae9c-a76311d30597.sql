...
-- Update execute_game_move: drap pool 2→5, rebalance tool probabilities
...
BEGIN
  ...
  IF NOT _is_story AND (_action = 'look' OR _action = 'confirm') AND _target_item_id IS NOT NULL AND _target_position IS NOT NULL THEN
    ...
    IF _tool_roll < 0.25 THEN
      -- Rebalanced: martell 10%, drap 8%, tornavis 4%, llanterna 3%
...
-- Update execute_tag_action: drap pool 2→5, rebalance tool probabilities
...
-- Update execute_toggle_light: drap pool 2→5, rebalance tool probabilities
