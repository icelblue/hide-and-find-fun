-- Migració 1: Jardí, mascota (acariciar), buy_room segur i participació recompensada
...
INSERT INTO public.garden_catalog (id, icon, name_key, growth_minutes, yield_coins) VALUES
  ('pastanaga', '🥕', 'garden.carrot',      240,  1.0),
  ('tomaquet',  '🍅', 'garden.tomato',      480,  2.0),
  ('maduixa',   '🍓', 'garden.strawberry', 1440,  4.0);
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(_room.layout) e WHERE (e->>'slot')::int = _slot) THEN
...
BEGIN
  ...
  IF _state.last_petted_at IS NOT NULL AND now() < _state.last_petted_at + interval '4 hours' THEN
    RETURN jsonb_build_object('bond', _state.bond, 'petted', false,
...
-- Migració 2: pulse realtime al fer un moviment (garanteix que
-- el rival rebi l'actualització encara que games no es toqui).
