...
ALTER TYPE social_item_type ADD VALUE IF NOT EXISTS 'barricada';
ALTER TYPE social_item_type ADD VALUE IF NOT EXISTS 'trampa';

-- 2. Add columns to game_social_items for barricada/trampa data
-- barricada: stores the blocked connection (scenario_a -> scenario_b)
-- trampa: stores the trapped item_id
ALTER TABLE game_social_items ADD COLUMN IF NOT EXISTS target_data jsonb DEFAULT NULL;

-- 3. RPC: Execute Barricada
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM game_players WHERE game_id = _game_id AND user_id = _rival.user_id AND shield_active) THEN
    ...
    UPDATE game_players SET social_item_used_today = true WHERE id = _player.id;
...
-- 5. Update execute_game_move to check traps and barricades
...
BEGIN
  ...
  ELSE
    ...
    IF _action = 'move' AND _target_scenario_id IS NOT NULL THEN
      ...
      IF _barricade.id IS NOT NULL THEN
        -- Barricade found! Player must pay extra cost
  ...
  IF _action = 'move' AND _target_scenario_id IS NOT NULL THEN
    ...
    -- Decrement barricade remaining_moves for all barricades targeting this player
    UPDATE game_social_items
    SET target_data = jsonb_set(target_data, '{remaining_moves}',
      to_jsonb(GREATEST(0, (target_data->>'remaining_moves')::int - 1)))
