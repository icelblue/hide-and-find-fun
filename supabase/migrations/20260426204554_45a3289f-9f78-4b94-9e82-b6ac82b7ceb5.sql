...
BEGIN
  ...
  ELSE
    ...
    -- Check barricade on move (only PvP). Single-use toll: pay +1🪙 once, then it falls.
    IF _action = 'move' AND _target_scenario_id IS NOT NULL THEN
      ...
      IF _barricade.id IS NOT NULL THEN
        ...
        _barricade_hit := true;
        -- Single-use: drop to 0 after one forced pass
  ...
  -- If confirmed correctly: finish game
  IF _action = 'confirm' AND _found_object THEN
