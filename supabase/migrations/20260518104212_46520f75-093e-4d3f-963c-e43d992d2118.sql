DROP FUNCTION IF EXISTS public.execute_game_move(uuid, action_type, uuid, uuid, position_type, boolean);

CREATE OR REPLACE FUNCTION public.execute_game_move(
  _game_id uuid,
  _action action_type,
  _target_scenario_id uuid,
  _target_item_id uuid,
  _target_position position_type,
...
BEGIN
  ...
  IF _action = 'look' AND _rival.id IS NOT NULL THEN
    ...
    ELSIF _rival.hidden_item_id = _target_item_id THEN
      _hint_level := 2;
    ELSIF _target_item_scenario IS NOT NULL AND _target_item_scenario = (SELECT scenario_id FROM items WHERE id = _rival.hidden_item_id) THEN
      _hint_level := 1;
    ELSE
      _hint_level := 0;
    END IF;

    -- HINT NOISE (Option C): 10% chance to nudge hint to an adjacent level.
    -- Never alters a true "found" (level 3) so victory is always honest.
    IF NOT _found_object AND _hint_level IS NOT NULL AND random() < 0.10 THEN
      IF _hint_level = 0 THEN
        _hint_level := 1;
        _hint_noisy := true;
      ELSIF _hint_level = 2 THEN
        _hint_level := 1;
        _hint_noisy := true;
      ELSIF _hint_level = 1 THEN
        IF random() < 0.5 THEN _hint_level := 0; ELSE _hint_level := 2; END IF;
        _hint_noisy := true;
      END IF;
    END IF;
...
-- Update furniture: items that logically can't have "dins"
UPDATE public.items SET inner_capacity = 0
WHERE name IN ('Arbre', 'Estenedor', 'Prestatge', 'Prestatgeria', 'Sofà');